...
parser.add_argument('--temperature', type=float, default=0, help="The temperature of the model. Increasing the temperature will make the model answer more creatively. (Default: 0.8)")
...
parser.add_argument('--min_p', type=float, default=0.0, help="Alternative to the top_p, and aims to ensure a balance of quality and variety. The parameter p represents the minimum probability for a token to be considered, relative to the probability of the most likely token. For example, with p=0.05 and the most likely token having a probability of 0.9, logits with a value less than 0.045 are filtered out. (Default: 0.0)")
...
with open("prompt_api.md", 'r', encoding='utf-8') as f:
    prompt = f.read()
...
with open(os.path.join(output_dir, f"output_{formatted_time}"), 'w') as f:
        f.write(output)
